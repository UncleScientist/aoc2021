...
pub fn day23a() {
    let e: Rooms = [[A, A, A, A], [B, B, B, B], [C, C, C, C], [D, D, D, D]];
    // let r: Rooms = [[B, D, D, A], [C, C, B, D], [B, B, A, C], [D, A, C, A]]; // test data
    let r: Rooms = [[B, D, D, C], [C, C, B, D], [A, B, A, D], [B, A, C, A]]; // given data
    let h: Hallway = [Empty; 11];
    let s: State = State(r, h);
    let end_condition = State(e, h);

    /*
    let tr: Rooms = [[Empty, Empty, A, A], [B, B, B, B], [C, C, C, C], [D, D, D, D]];
    let th: Hallway = [A,Empty,Empty,Empty,Empty,Empty,Empty,Empty,Empty,Empty,A];
    let ts = State(tr, th);
    ts.print();
    ...
    while !qset.is_empty() {
        ...
        if found == end_condition {
            ...
            while prev.contains_key(&u) {
                let v = prev.get(&u).unwrap();
                v.print();
                u = *v;
            }
            */
            println!("Day 23 - Part 2: {} ", smallest);
            break;
        }
